...
"""
Base script for running FSB datasets without noise in training data
"""


def drop_noisy_sections(samples: np.ndarray, sample_hist: np.ndarray,
                        sample_dates: np.ndarray, sample_anomalies: np.ndarray):
    """
    Drops sections with anomalies from the samples, sample_hist, sample_dates, and sample_anomalies.
    :param samples: Samples to filter.
    :param sample_hist: History of samples to filter.
    :param sample_dates: Dates of samples to filter.
    :param sample_anomalies: Anomalies in samples to filter.
    :return: Tuple of filtered samples, sample_hist, sample_dates, and sample_anomalies.
    """
    # indexes where sample_anomalies is 1
    anomaly_indexes = np.where(sample_anomalies == 1)[0]
    if len(anomaly_indexes) == 0:
        logger.info("No anomalies found in samples, skipping filtering")
        return samples, sample_hist, sample_dates, sample_anomalies
    # group the indexes by consecutive sections
    sections = np.split(anomaly_indexes, np.where(np.diff(anomaly_indexes) != 1)[0] + 1)
    # extend each section by the length of hist
    hist_length = sample_hist.shape[1]
    sections = [np.arange(section[0], section[-1] + hist_length) for section in sections]
    # create a mask for the samples
    mask = np.ones(samples.shape[0], dtype=bool)
    for section in sections:
        # limit the section to the length of samples
        section = section[section < samples.shape[0]]
        mask[section] = False
    # filter the samples, sample_hist, sample_dates, and sample_anomalies
    filtered_samples = samples[mask]
    filtered_sample_hist = sample_hist[mask]
    filtered_sample_dates = sample_dates[mask]
    filtered_sample_anomalies = sample_anomalies[mask]

    logger.info(f"Dropped {samples.shape[0] - filtered_samples.shape[0]} anomaly sections from samples")
    logger.info(f"Remaining samples: {filtered_samples.shape[0]}")
    return filtered_samples, filtered_sample_hist, filtered_sample_dates, filtered_sample_anomalies
...
def run(run_args=None, full_config=None, generator=(), device=None, **kwargs):
    ...
    if "no-anomaly" in sequence_name:
        ...
        logger.info("Skipping no-anomaly in sequence")
        return
    ...
    try:
        ...
        # drop sections with anomalies from samples
        samples, sample_hist, sample_dates, sample_anomalies = drop_noisy_sections(samples, sample_hist, sample_dates,
                                                                                   sample_anomalies)
