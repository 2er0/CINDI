...
"""
Base script for running FSB datasets with fixed interpolation methods and model selection
"""
...
def interpolate_with_method(train_sequence_source: pd.DataFrame, interpolation_method: str):
    ...
    if interpolation_method == "linear":
        return train_sequence.interpolate(method="linear")
    elif interpolation_method == "nearest":
        return train_sequence.interpolate(method="nearest")
    elif interpolation_method == "slinear":
        return train_sequence.interpolate(method="slinear")
    elif interpolation_method == "quadratic":
        return train_sequence.interpolate(method="quadratic")
    elif interpolation_method == "cubic":
        return train_sequence.interpolate(method="cubic")
    elif interpolation_method == "cubicspline":
        return train_sequence.interpolate(method="cubicspline")
    else:
...
if __name__ == "__main__":
    ...
    current_args = {"dataset": {"sequence_name": "2-sine-one-channel-anomaly-noise-01%"}}
