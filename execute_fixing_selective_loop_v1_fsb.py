...
"""
Base script for running mTADS datasets with model interpolation methods and sanity check model selection and 
...
def run(run_args=None, full_config=None, generator=(), device=None, **kwargs):
    ...
    logger.info(f"Loaded sequence: {sequence_name}")
    ...
    logger.info(f"Using optimization metric: {optimization_metric}")
    ...
    while run_fixing:
        ...
        try:
            ...
            if iteration < fix_iterations and len(to_fix_entries) > 0:
                ...
                run_fixing = True
                last_run = True
                # already on detection or best mode, stop here
                break
...
if __name__ == "__main__":
    ...
    --project="TFselfopt_fix_selective_loop_v1-fsb-testing"
    --dataset="fsb"
    ...
    --sanity_check=True
    --use_checks_in_optimization=True
